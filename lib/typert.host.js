...
const tokenScopeParamSchema = z.object({ scope: z.string().optional() })
const tokenRowsSchema = z.object({ scope: z.string(), rows: z.array(z.unknown()), total: z.unknown() })
const tokenSessionSchema = z.object({ rows: z.array(z.unknown()), total: z.unknown() })
...
export const TYPERT = {
  ...
  invocations: [
    ...
    },
    {
      ...
      invocation: { kind: 'direct' },
      parameters: [mkParam('request', emptyParamSchema, PKG + '/types#EmptyRequest')],
      result: mkResult(PKG + '/types#TokenSummaryResult', tokenSummarySchema),
      sourceLocation: { file: 'lib/token-stats.host.js', line: 1, column: 1 },
    },
    {
      id: PKG + '#tokenStats/breakdown',
      service: 'tokenStats',
      namespace: 'tokenStats',
      method: 'breakdown',
      invocation: { kind: 'direct' },
      parameters: [mkParam('request', tokenScopeParamSchema, PKG + '/types#TokenScopeRequest')],
      result: mkResult(PKG + '/types#TokenRowsResult', tokenRowsSchema),
      sourceLocation: { file: 'lib/token-stats.host.js', line: 1, column: 1 },
    },
    {
      ...
      invocation: { kind: 'direct' },
      parameters: [mkParam('request', emptyParamSchema, PKG + '/types#EmptyRequest')],
      result: mkResult(PKG + '/types#TokenSessionResult', tokenSessionSchema),
      sourceLocation: { file: 'lib/token-stats.host.js', line: 1, column: 1 },
    },
    {
  ...
  model: {
    services: [
      ...
      {
        description: 'Token usage statistics (week / month / session).',
        summary: 'Token stats remote service.',
        tags: [],
        jsDoc: '/** Token stats */',
        key: 'tokenStats',
        exportName: 'TokenStatsService',
        members: [
          { kind: 'method', name: 'summary', signature: 'summary(): Promise<TokenSummaryResult>', summary: 'Week/month/session usage summaries.' },
          { kind: 'method', name: 'breakdown', signature: 'breakdown(request: TokenScopeRequest): Promise<TokenRowsResult>', summary: 'Per provider/model rows for a scope.' },
          { kind: 'method', name: 'session', signature: 'session(): Promise<TokenSessionResult>', summary: 'Session-scope usage rows.' },
          { kind: 'method', name: 'reset', signature: 'reset(): Promise<SaveResult>', summary: 'Clear all usage stats.' },
        ],
        types: [
          { name: 'TokenSummaryResult', declaration: 'export interface TokenSummaryResult { readonly week: unknown; readonly month: unknown; readonly session: unknown; readonly startedAt: number; readonly dataFile: string }' },
          { name: 'TokenScopeRequest', declaration: 'export interface TokenScopeRequest { readonly scope?: string }' },
          { name: 'TokenRowsResult', declaration: 'export interface TokenRowsResult { readonly scope: string; readonly rows: unknown[]; readonly total: unknown }' },
          { name: 'TokenSessionResult', declaration: 'export interface TokenSessionResult { readonly rows: unknown[]; readonly total: unknown }' },
        ],
      },
    ],
    events: [],
